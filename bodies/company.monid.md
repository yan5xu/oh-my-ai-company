...
本轮按产品 onboarding 在 `pinixc browser` default profile 中选择 Codex、进入注册。账号创建页支持 GitHub、Google、email；点击 Google 后触发 Cloudflare “Verify you are human”。该验证需要用户手动完成，本轮没有脚本绕过。[[source.monid.app-onboarding-2026-07-22]]

![Monid signup](../assets/monid/app-signup-2026-07-22.png)

因此当前可确认的是：公开目录、Docs、API contract、CLI repo 和注册入口真实存在；**免费 `$1` credit、API key、budget、真实 run、扣费/失败不扣费/退款闭环尚未完成账户内验证**。
